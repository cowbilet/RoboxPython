...
while True:
    for comm in communications:
        ...
        if line:
            if (DEBUG):
                print("Received over {}: {}".format(comm.name, line))
            command = COMMANDS.get(line)
            if command == "firmware_check":
                if DEBUG:
                    print("Firmware check over {}".format(comm.name))
                ...
                if comm == usb:
                    if ble in communications:
                        if DEBUG:
                            print("Putting BLE to sleep")
                        ble.sleep()
                current_communication_method = comm
                comm.write_message("console", "Firmware OK")
                ...
                if (DEBUG):
                    print("Beginning upload over {}".format(comm.name))
                time.sleep(0.1)
                ...
                if DEBUG:
                    print("Writing to file over {}: {}".format(comm.name, line))
                LED.toggle()
                out_file.write(line + "\n")
